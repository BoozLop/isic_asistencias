...
// Inicio de funcion para comprobar que no se repita el nombre de usuario al guardarlo ---------------------------
function CompararUsuario(nombre_usuario){
    $.ajax({
        url:"../mUsuarios/comprobarUsuario.php",
        type:"POST",
        dateType:"html",
        data:{nombre_usuario},
        success:function(respuesta){
...
// Fin de funcion para comprobar que no se repita el nombre de usuario al guardarlo -------------------------------
...
function llenar_formulario_U(id,nom_usuario,nom_persona,nom_tema,p_datos_persona,p_ecivil,p_usuarios,p_temas,fechaCadu){
    ...
    if (p_datos_persona == 'si') {
        $("#eP_datosP").bootstrapToggle('on');
    } else {
        $("#eP_datosP").bootstrapToggle('off');
    }
    if (p_ecivil == 'si') {
        $("#eP_Ecivil").bootstrapToggle('on');
    } else {
        $("#eP_Ecivil").bootstrapToggle('off');
    }
    if (p_usuarios == 'si') {
        $("#eP_usuarios").bootstrapToggle('on');
    } else {
        $("#eP_usuarios").bootstrapToggle('off');
    }
    if (p_temas == 'si') {
        $("#eP_temas").bootstrapToggle('on');
    } else {
        $("#eP_temas").bootstrapToggle('off');
    }
...
function cambiar_estatus_U(id,consecutivo){
    ...
    $.ajax({
        ...
        success:function(respuesta){
            // console.log(respuesta);
            if(contravalor==1){
                alertify.success("<i class='fa fa-check fa-lg'> Activado</i>", 2);
                ...
                $("#icoContra-U"+consecutivo).removeClass("fas fa-sync fa-lg");
                $("#icoContra-U"+consecutivo).addClass("fas fa-sync fa-spin fa-lg");
                $("#icoPermisos-U"+consecutivo).removeClass("fas fa-lock fa-lg");
                $("#icoPermisos-U"+consecutivo).addClass("fas fa-key fa-lg");
                ...
                $("#icoContra-U"+consecutivo).removeClass("fas fa-sync fa-spin fa-lg");
                $("#icoContra-U"+consecutivo).addClass("fas fa-sync fa-lg");
                $("#icoPermisos-U"+consecutivo).removeClass("fas fa-key fa-lg");
                $("#icoPermisos-U"+consecutivo).addClass("fas fa-lock fa-lg");
...
function abrirModalPermisos(id,persona,datos_p,ecivil,usuarios,temas){
    ...
    if (datos_p == 'si') {
        $("#mP_datosP").bootstrapToggle('on');
    } else {
        $("#mP_datosP").bootstrapToggle('off');
    }
    if (ecivil == 'si') {
        $("#mP_Ecivil").bootstrapToggle('on');
    } else {
        $("#mP_Ecivil").bootstrapToggle('off');
    }
    if (usuarios == 'si') {
        $("#mP_usuarios").bootstrapToggle('on');
    } else {
        $("#mP_usuarios").bootstrapToggle('off');
    }
    if (temas == 'si') {
        $("#mP_temas").bootstrapToggle('on');
    } else {
        $("#mP_temas").bootstrapToggle('off');
    }
    ...
    swal({
        ...
        setTimeout(function () {
                $.ajax({
                    ...
                    success : function(respuesta) {
                        swal.close();
                        alertify.success("<i class='fa fa-check fa-lg'> Permisos Actualizados</i>", 2);
                        llenar_lista_U();
                        var idTema=$("#inicioIdTema").val();
                        aplicarTema(idTema,'otro');
...
{
    $.ajax({
        url : '../mUsuarios/comboPersonaG.php',
        data : {},
        type : 'POST',
        dataType : 'html',
        success : function(respuesta) {
...
function combo_personasE()
{
    $.ajax({
        url : '../mUsuarios/comboPersonaE.php',
        data : {},
        type : 'POST',
        dataType : 'html',
        success : function(respuesta) {
