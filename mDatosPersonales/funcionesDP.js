...
// --------------------------------------------Modal Horario-------------------------------------- //
function abrirModalHorario(id, nombre, turno, L_entrada, L_salida, Mar_entrada, Mar_salida, Mier_entrada, Mier_salida, J_entrada, J_salida, V_entrada, V_salida, S_entrada, S_salida, D_entrada, D_salida, tHorario) {
    $("#modalTitle-Horario").text("Horario - "+nombre);

    $("#id_persona").val(id);
    // $("#Turno").val(turno);
    // $("#L_entrada").val(L_entrada);
    // $("#L_salida").val(L_salida);
    // $("#Mar_entrada").val(Mar_entrada);
    // $("#Mar_salida").val(Mar_salida);
    // $("#Mier_entrada").val(Mier_entrada);
    // $("#Mier_salida").val(Mier_salida);
    // $("#J_entrada").val(J_entrada);
    // $("#J_salida").val(J_salida);
    // $("#V_entrada").val(V_entrada);
    // $("#V_salida").val(V_salida);
    // $("#S_entrada").val(S_entrada);
    // $("#S_salida").val(S_salida);
    // $("#D_entrada").val(D_entrada);
    // $("#D_salida").val(D_salida);
    $("#Nombre_Persona").val(nombre);
    $("#tHorario").val(tHorario);
    ...
    if (tHorario == 'Si') {
        ...
        if (turno == "Especial") {
            ...
            if (turno == "Matutino") {
                ...
                $("#Turno").val("TMatutino");
                $("#btnReiniciarH").attr('disabled','disabled');
...
// Inicio de funcion para el cambio de valores de los inputs dependiendo del turno seleccionado -------------- //
$("#Turno").change(function(){
    var valorCombo = $(this).val();

    if (valorCombo == 'Matutino') {
        $("#L_entrada").val("06:00:00");
        $("#L_salida").val("12:00:00");
        $("#Mar_entrada").val("06:00:00");
        $("#Mar_salida").val("12:00:00");
        $("#Mier_entrada").val("06:00:00");
        $("#Mier_salida").val("12:00:00");
        $("#J_entrada").val("06:00:00");
        $("#J_salida").val("12:00:00");
        $("#V_entrada").val("06:00:00");
        $("#V_salida").val("12:00:00");
        $("#S_entrada").val("--:--:--");
        $("#S_salida").val("--:--:--");
        $("#D_entrada").val("--:--:--");
        $("#D_salida").val("--:--:--");
        $("#HorasTrabajadas").val("30");
        $("#HorasTrabajadas").css('background','rgba(46, 204, 113,0.2)');
        $("#btnGuardarHorario").removeAttr('disabled');
        $("#btnReiniciarH").attr('disabled','disabled');
        $("#L_entrada,#L_salida,#Mar_entrada,#Mar_salida,#Mier_entrada,#Mier_salida,#J_entrada,#J_salida,#V_entrada,#V_salida,#S_entrada,#S_salida,#D_entrada,#D_salida").attr('disabled','disabled');
    }else{
      if (valorCombo == 'Vespertino') {
            $("#L_entrada").val("12:00:00");
            $("#L_salida").val("18:00:00");
            $("#Mar_entrada").val("12:00:00");
            $("#Mar_salida").val("18:00:00");
            $("#Mier_entrada").val("12:00:00");
            $("#Mier_salida").val("18:00:00");
            $("#J_entrada").val("12:00:00");
            $("#J_salida").val("18:00:00");
            $("#V_entrada").val("12:00:00");
            $("#V_salida").val("18:00:00");
            $("#S_entrada").val("--:--:--");
            $("#S_salida").val("--:--:--");
            $("#D_entrada").val("--:--:--");
            $("#D_salida").val("--:--:--");
          ...
          if(valorCombo == 'Nocturno'){
                $("#L_entrada").val("18:00:00");
                $("#L_salida").val("00:00:00");
                $("#Mar_entrada").val("18:00:00");
                $("#Mar_salida").val("00:00:00");
                $("#Mier_entrada").val("18:00:00");
                $("#Mier_salida").val("00:00:00");
                $("#J_entrada").val("18:00:00");
                $("#J_salida").val("00:00:00");
                $("#V_entrada").val("18:00:00");
                $("#V_salida").val("00:00:00");
                $("#S_entrada").val("--:--:--");
                $("#S_salida").val("--:--:--");
                $("#D_entrada").val("--:--:--");
                $("#D_salida").val("--:--:--");
              ...
              if (valorCombo == 'Especial') {
                    $("#L_entrada").val("00:00:00");
                    $("#L_salida").val("00:00:00");
                    $("#Mar_entrada").val("00:00:00");
                    $("#Mar_salida").val("00:00:00");
                    $("#Mier_entrada").val("00:00:00");
                    $("#Mier_salida").val("00:00:00");
                    $("#J_entrada").val("00:00:00");
                    $("#J_salida").val("00:00:00");
                    $("#V_entrada").val("00:00:00");
                    $("#V_salida").val("00:00:00");
                    $("#S_entrada").val("00:00:00");
                    $("#S_salida").val("00:00:00");
                    $("#D_entrada").val("00:00:00");
                    $("#D_salida").val("00:00:00");
                    $("#HorasTrabajadas").val("0");
                    $("#HorasTrabajadas").css('background','rgba(231, 76, 60,0.2)');
...
// Fin de funcion para el cambio de valores de los inputs dependiendo del turno seleccionado -------------- //

// Inicio de evento onclick para reiniciar los inputs en caso que el usuario lo desee si es turno especial--- //
$("#btnReiniciarH").click(function(){
    $("#L_entrada").val("00:00:00");
    $("#L_salida").val("00:00:00");
    $("#Mar_entrada").val("00:00:00");
    $("#Mar_salida").val("00:00:00");
...
// Fin de evento onclick para reiniciar los inputs en caso que el usuario lo desee si es turno especial----- //

// Inicio de funcion para sacar la suma de horas en toda la semana ----------------------------------- //
function contadorHoras() {
    ...
    if (sumaHoras >= 30) {
        $("#btnGuardarHorario").removeAttr('disabled');
        $("#HorasTrabajadas").css('background','rgba(46, 204, 113,0.2)');
        $("#btnReiniciarH").removeAttr('disabled');
    } else {
        $("#btnGuardarHorario").attr('disabled','disabled');
        $("#HorasTrabajadas").css('background','rgba(231, 76, 60,0.2)');
        $("#btnReiniciarH").attr('disabled','disabled');
...
// Fin de funcion para sacar la suma de horas en toda la semana ----------------------------------- //

// Inicio de evento keyup cuando cambian los valores dentro de los inputs tanto de entrada como de salida ------------------------- //
...
// Fin de evento keyup cuando cambian los valores dentro de los inputs tanto de entrada como de salida ------------------------- //

// Inicio de funcion para guardar o actualizar el horario de una persona --------------------------------------- //
function GuardarHorario(){
    var tHorario       = $("#tHorario").val();
...
// Fin de funcion para guardar o actualizar el horario de una persona --------------------------------------------//